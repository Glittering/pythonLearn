...
b = ['aa','BB','bb','zz','CC'] 
sorted(b) 
['BB', 'CC', 'aa', 'bb', 'zz']    #按列表中元素每个字母的ascii码从小到大排序,
                                  #如果要从大到小,请用sorted(b,reverse=True)下同 
...
d =['CCC', 'bb', 'ffff', 'z'] 
sorted(d,key = str.lower )    #将列表中的每个元素变为小写，
                              #再按每个元素中的每个字母的ascii码从小到大排序 
['bb', 'CCC', 'ffff', 'z'] 
...
['b', 'abc', 'ef', 'AAz']     #sorted(e,key=lastchar)作用就是
...
f2 = sorted(f,key = lambda x:x['age'])    #如果觉得上面定义一个函数代码不美观，
                                          #可以用lambda的形式来定义函数,效果同上 
